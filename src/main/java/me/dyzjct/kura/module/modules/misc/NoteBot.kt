...
import me.dyzjct.kura.event.events.client.SettingChangeEvent
...
import me.dyzjct.kura.gui.settingpanel.component.components.Button
import me.dyzjct.kura.gui.settingpanel.component.components.ScrollPane
import me.dyzjct.kura.gui.settingpanel.layout.GridLayout
import me.dyzjct.kura.gui.window.WindowChooser
import me.dyzjct.kura.manager.FileManager
import me.dyzjct.kura.module.Category
import me.dyzjct.kura.module.Module
import me.dyzjct.kura.utils.NTMiku.MultiThreading.runAsync
...
class NoteBot : Module() {
    ...
    inner class SongChooser : GuiScreen() {
        ...
        private var window = WindowChooser("Choose Your Song", 150, 150, 290, 347)
        ...
        private fun reload() {
            val pane = ScrollPane(GridLayout(1))
            var c = 0
            for (song in Objects.requireNonNull(file.listFiles())) {
                if (song.isDirectory || !song.name.endsWith(".notebot")) continue
                val but = Button(song.name.replace(".notebot", ""), 280, 15)
                but.onClickListener = ActionEventListener {
                    try {
