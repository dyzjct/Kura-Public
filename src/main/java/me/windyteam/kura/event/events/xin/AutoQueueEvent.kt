package me.windyteam.kura.event.events.xin
...
class AutoQueueEvent : Event() {
    ...
    fun onReceiveChat(event: ClientChatReceivedEvent) {
        try {
            ...
            } else if (event.message.toString().contains("小箱子能存储多少格物品")) {
                if (event.message.toString().contains("A.27")) {
                    mc.player.sendChatMessage("A")
                } else if (event.message.toString().contains("B.27")) {
                    mc.player.sendChatMessage("B")
            ...
            } else if (event.message.toString().contains("苦力怕的官方译名")) {
                if (event.message.toString().contains("A.爬行者")) {
                    mc.player.sendChatMessage("A")
                } else if (event.message.toString().contains("B.爬行者")) {
                    mc.player.sendChatMessage("B")
                } else if (event.message.toString().contains("C.爬行者")) {
                    mc.player.sendChatMessage("C")
                }
            }
